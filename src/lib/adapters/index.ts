import { int, timestamp, mysqlTable, varchar, primaryKey, text } from 'drizzle-orm/mysql-core'
...
export function PlanetScaleAdapter(client: DbClient, schema?: Partial<CustomSchema>): Adapter {
  ...
  return {
    createUser: async (data) => {
      ...
      await client.insert(users).values({ id, email: data.email, name: data.name })
    ...
    updateUser: async (data) => {
      ...
      const user = await client.select().from(users).where(eq(users.id, data.id))
      return user[0]
    },
